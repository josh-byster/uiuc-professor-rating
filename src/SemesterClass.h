...
struct SemesterClass {
    ...
    SemesterClass(unsigned int year, std::string term, std::string subject, unsigned int courseNumber) : year(year),
                                                                                                         subject(subject),
...
namespace std {
    ...
    struct hash<SemesterClass> {
        std::size_t operator()(const SemesterClass &value) const {
            ...
            return (hash<int>()(value.year) * hash<int>()(value.term) + hash<int>()(value.courseNumber) +
                    hash<string>()(value.subject));
        }
    };
